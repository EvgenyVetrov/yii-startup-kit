...
class RolesForm extends Model
{
    ...
    private function saveRole()
    {
        ...
        if ($this->scenario == self::SCENARIO_UPDATE) {
            Log::create(Log::TYPE_UPDATE, 'UPDATE_RBAC_ROLE_{alias}', ['alias' => $this->alias],
                $this->oldAttributes,
                $this->attributes,
                ['name', 'alias', 'rule', 'permissions', 'child_roles']
            );
        }else{
            Log::create(Log::TYPE_INSERT, 'CREATE_RBAC_ROLE_{alias}', ['alias' => $this->alias],
                $this->oldAttributes,
                $this->attributes,
                ['name', 'alias', 'rule', 'permissions', 'child_roles']
            );
    ...
    public function delete()
    {
        ...
        $rule = $auth->getRole($this->alias);

        if ($auth->remove($rule)){
            Log::create(Log::TYPE_DELETE, 'DELETE_RBAC_ROLE_{alias}', ['alias' => $this->alias],
                $this->oldAttributes,
                $this->attributes,
                ['name', 'alias', 'rule', 'permissions', 'child_roles']
            );
    ...
    /**
     * Получаем права доступа данной роли
     * @return array
     */
    public function loadPermissions()
    {
        $values = [];
        $array  = $this->getPermissions();

        foreach ($array as $key => $value){
            if (self::isChild($this->alias, $key)){
                $values[] = $key;
            }
        }
